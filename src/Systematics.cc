...
Systematics::Systematics(int random_seed, std::vector<std::string> vars)
  : _ok(true),
	...
	_nrow(0)
{
//	cout << "Random Number Seed = " << random_seed << endl;
...
//  cout << "pt3: " << lepton[2].pt << endl;
  ...
  // Data/MC correction
  float datamcweight = getMCWeight();
...
{
	TRandom myRandom(_random_seed);
	for(int i = 0; i < _nsysts; i++) _syst_rand.push_back( myRandom.Gaus(0.0, 1.0) );
	_data_to_mc_random_syst_mu = myRandom.Gaus(0.0, 1.0);
	_data_to_mc_random_stat_mu = myRandom.Gaus(0.0, 1.0);
	_data_to_mc_random_reco_e = myRandom.Gaus(0.0, 1.0);
	_data_to_mc_random_id_e = myRandom.Gaus(0.0, 1.0);
	_pt_scale_random = myRandom.Gaus(0.0, 1.0);
	_electron_Et_scale_random = myRandom.Gaus(0.0, 1.0);
	_muon_pt_scale_random = myRandom.Gaus(0.0, 1.0);
}
